...
public class BoardDisplayer {
	 ...
	 */
	public static void drawLine(Graphics g, Point initialposition, int height, int width, Orientation Orientation){
		if(Orientation != null){
	...
	public static void  drawPanel(Graphics g, Board t,Point initialCoordinate ,Point panelCoordinate, int panelHeight, int panelWidth){
		...
		for(Orientation d : borderOrientations){
			if ((getPanelInOrientation(panelCoordinate, t, d) == null) && (d != null))
		...
		for(Orientation tmpOrientation : nullOrientations){
			if (tmpOrientation != null){
				drawLine(g,initialCoord, panelHeight, panelWidth, tmpOrientation);
				for (Orientation tmp : nullOrientations){
					if ((tmpOrientation.opposite() != tmp)&&(tmp != tmpOrientation)&&(tmp != null))
						drawCorner(g, t,initialCoordinate, panelCoordinate, panelHeight, panelWidth, tmp, tmpOrientation, false);
				}
			}
		}
	 ...
	 * @param Orientation2
	 */
	public static void drawCorner(Graphics g, Board t, Point initialCoordinate, Point panelCoordinate, int panelHeight, int panelWidth, Orientation Orientation1, Orientation Orientation2, boolean inner){
		Color original = g.getColor();
		g.setColor(color);
		Point startCoord = new Point(0, 0);
		int startAngle = 0;
		if ((Orientation1 == Orientation.NORTH && Orientation2 == Orientation.WEST) || (Orientation1 == Orientation.WEST && Orientation2 == Orientation.NORTH)){
			startCoord = new Point(initialCoordinate.x + (panelCoordinate.x * panelWidth), initialCoordinate.y + (panelCoordinate.y * panelHeight));
			startAngle = 180;
		}
		if ((Orientation1 == Orientation.NORTH && Orientation2 == Orientation.EAST) || (Orientation1 == Orientation.EAST && Orientation2 == Orientation.NORTH))
		{
		...
		if ((Orientation1 == Orientation.SOUTH && Orientation2 == Orientation.WEST) || (Orientation1 == Orientation.WEST && Orientation2 == Orientation.SOUTH))
		{
		...
		if ((Orientation1 == Orientation.SOUTH && Orientation2 == Orientation.EAST) || (Orientation1 == Orientation.EAST && Orientation2 == Orientation.SOUTH))
		{
			startCoord = new Point(initialCoordinate.x + (panelWidth - (2 * (panelWidth/AWAYFROMBORDER))) + (panelCoordinate.x * panelWidth),
					initialCoordinate.y + panelHeight - (2 * (panelHeight/AWAYFROMBORDER)) + (panelCoordinate.y * panelHeight));
			startAngle = 0;
	...
	public static void drawReverseCorner(Graphics g, Board t, Point initialCoord, Point panelCoord, int panelHeight, int panelWidth, Orientation Orientation1, Orientation Orientation2, boolean nullCorner){
		Color original = g.getColor();
		g.setColor(color);
		Point startCoord = new Point(0, 0);
		int startAngle = 0;
	...
	/**
	 * This method returns the panel in the given Orientation of the given coordinate on the given track.
	 * @param 	panelCoord
	 ...
	 * @return
	 * 			The panel in the given Orientation.
	 * @note	The panel could be null.
	 */
	private static Panel getPanelInOrientation(Point panelCoord, Board t, Orientation Orientation){
		if (Orientation == null)
			return null;
