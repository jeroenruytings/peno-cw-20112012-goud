...
public class SimRobotDataDisplay extends Canvas{
	...
	private Color robotColor = Color.PINK;
	 ...
	 * @param 	name
	 * 				Name of the image (with the extention).
	 * 				The file must be in the resources package, of this project.
	 * @return
	 * 			Image object, representing the file.
	 */
	...
	private void drawRobot(Graphics g){
		if (myRobotData.getPosition() != null){
			g.drawImage(getImage("ghost.png"),(int)(calculateInitialPosition().getX() + (calculatePanelWidth() * myRobotData.getPosition().getX()) + getSpacing()),
					(int)(calculateInitialPosition().getY() + (calculatePanelWidth() * myRobotData.getPosition().getY()) + getSpacing()), 
	...
	private void drawPacman(Graphics g){
		if (myRobotData.getPacmanLastSighted() != null){
			...
			g.fillOval((int)(calculateInitialPosition().getX() + (calculatePanelWidth() * myRobotData.getPacmanLastSighted().getX()) + getSpacing()),
					(int)(calculateInitialPosition().getY() + (calculatePanelWidth() * myRobotData.getPacmanLastSighted().getY()) + getSpacing()), 
					calculatePanelWidth() - 2 * getSpacing(), 
					calculatePanelWidth() - 2 * getSpacing());
			g.setColor(getBackground());
			g.fillArc((int)(calculateInitialPosition().getX() + (calculatePanelWidth() * myRobotData.getPacmanLastSighted().getX()) + getSpacing()),
					(int)(calculateInitialPosition().getY() + (calculatePanelWidth() * myRobotData.getPacmanLastSighted().getY()) + getSpacing()), 
					calculatePanelWidth() - 2 * getSpacing(), 
					calculatePanelWidth() - 2 * getSpacing(),
					50,-80);
