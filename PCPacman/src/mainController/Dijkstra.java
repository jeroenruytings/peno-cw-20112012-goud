...
import mainController.EuclideanGraph.AdjListIterator;
...
public class Dijkstra {
    ...
    private void dijkstra(int s, int d) {
        ...
        while (!pq.isEmpty()) {
            ...
            AdjListIterator i = G.neighbors(v);
            while (i.hasNext()) {
                int w = i.next();
                Square sqw = G.getSquare(w);
                if (dist[v] + G.distance(sqv, sqw) < dist[w] - EPSILON) {
                    dist[w] = dist[v] + G.distance(sqv, sqw);
                    pq.change(w, dist[w]);
                    pred[w] = v;
